...
// with clerk integration?? idk, no hooks for sure:
// import { NextRequest, NextResponse } from "next/server"
...
//         await Promise.all(uploadPromises)
//
//         // FIXED: Update both Prisma AND Clerk
//         const avatarUrl = urls.avatar
//
...
//         })
//
//         // CRITICAL: Also update Clerk user metadata
...
//         if (currentUser?.image && currentUser.image.includes(PUBLIC_URL)) {
//             // Extract key from URL and delete old files
//             const oldKey = currentUser.image.replace(`https://${PUBLIC_URL}/`, '')
//             const baseOldKey = oldKey.replace('-avatar.webp', '')
//
//             // Delete all old avatar variations
//             Promise.all([
...
//         })
//
//         // CRITICAL: Also remove from Clerk
//         await clerkClient.users.updateUser(userId, {
//             imageUrl: undefined,
//         })
//
//         // Delete from R2 if it's our URL
//         if (user.image.includes(PUBLIC_URL)) {
//             const key = user.image.replace(`https://${PUBLIC_URL}/`, '')
//             const baseKey = key.replace('-avatar.webp', '')
//
//             // Delete all avatar variations
//             await Promise.all([
