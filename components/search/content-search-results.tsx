...
import { CaseStudySearchRecord, ReportSearchRecord } from '@/lib/algolia'
...
function ReportResult({ hit }: { hit: ReportSearchRecord }) {
  const getLocalizedTitle = (title: any) => {
    if (typeof title === 'string') return title
    return title?.en || title?.es || title?.fr || title?.ar || 'Untitled'
  }
            ...
            <div className="flex items-start justify-between mb-2">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <Badge variant="outline" className="text-xs">
                    {getReportTypeLabel(hit.reportType)}
                  </Badge>
                  <Badge variant="outline" className="text-xs">
                    {hit.year}
                  </Badge>
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-muted-foreground">
          {type === 'case-studies' ? 'Try different keywords or check back later for new case studies.' : 'Try different keywords or check back later for new reports.'}
        </p>
      </div>
    )
  }
  ...
  return (
    <div>
      <div className="space-y-4">
        ...
        {type === 'reports' && hits.map((hit) => (
          <ReportResult key={hit.objectID} hit={hit as unknown as ReportSearchRecord} />
        ))}
      </div>
