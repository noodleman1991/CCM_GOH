...
interface GridCaseStudyComponentProps {
    ...
    _key: string;
    caseStudy: CaseStudyModalData; // Loosely-projected Sanity data (see case-study-modal)
    showTags?: boolean;
    showAuthors?: boolean;
    showMetadata?: boolean;
    showStudyPeriod?: boolean;
    showLocation?: boolean;
    customExcerpt?: LocalizedString | string;
    customLayout?: string;
    locale: string;
    userId?: string;
    className?: string;
...
export default function GridCaseStudyComponent({
    ...
    // Plain render helper (not a component) so the react-compiler doesn't see a
    // component created during render — output is identical.
    const renderCoverImage = (imgWrapClass?: string) => (
    ...
    if (variant === "wide") {
        ...
        card = (
            <Card className={cn(cardBase, "h-full flex-col @content-sm/page:flex-row")} onClick={onCardClick}>
                {renderCoverImage("aspect-video w-full shrink-0 @content-sm/page:aspect-auto @content-sm/page:w-2/5")}
        ...
        card = (
            <Card className={cn(cardBase, "h-full flex-col")} onClick={onCardClick}>
                {renderCoverImage("aspect-[16/9] w-full")}
        ...
        card = (
            <Card className={cn(cardBase, "h-full flex-col justify-between p-6")} onClick={onCardClick}>
                ...
                {renderCoverImage("mb-4 aspect-video w-full rounded-2xl")}
                ...
                <CardContent className="flex-1 px-0 pb-3">
                    ...
                    {renderMeta()}
                    <div className="mt-3">{renderTags()}</div>
                </CardContent>
